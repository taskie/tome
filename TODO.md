...
- 宣言型 DDL — `docs/schema/tome-db.sql` を自動生成（`generate_schema` example）、DSQL 抽象化層を削除
...
- `store push` の直接リモート対応 — `store push` に `--encrypt/--key-file/--key-source/--cipher` フラグを追加。per-store config + CLI オーバーライドにより、ローカルストアを経由せず S3/SSH 等に直接暗号化 push 可能
