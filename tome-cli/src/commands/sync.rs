...
async fn sync_add(db: &DatabaseConnection, args: SyncAddArgs) -> Result<()> {
    ...
    println!(
        "sync peer registered: {} (id={}, url={}, peer_repo={})",
        peer.name, peer.id, peer.url, peer_repo
    );
    Ok(())
}
...
async fn sync_list(db: &DatabaseConnection, args: SyncListArgs) -> Result<()> {
    ...
    println!("{:<20} {:<20} {}", "name", "last_snapshot_id", "url");
    println!("{}", "-".repeat(70));
    for p in peers {
...
async fn sync_pull(local_db: &DatabaseConnection, args: SyncPullArgs) -> Result<()> {
    ...
    let new_snapshots =
        ops::snapshots_after(&peer_db, remote_repo.id, peer.last_snapshot_id).await?;
    ...
    for remote_snap in &new_snapshots {
        ...
        for remote_entry in &remote_entries {
            ...
            let local_blob_id = if let Some(ref remote_blob_id) = remote_entry.blob_id {
                ...
                let remote_blob =
                    match ops::find_blob_by_id(&peer_db, *remote_blob_id).await? {
                        Some(b) => b,
                        None => {
                ...
                let local_blob =
                    ops::find_blob_by_digest(local_db, &remote_blob.digest).await?;
            ...
            if remote_entry.status == 1 {
                if let Some(blob_id) = local_blob_id {
                    ...
                    ops::upsert_cache_present(
                        ...
                        blob_id,
                        remote_entry.mtime,
                        None, // digest not cached here; filled on next scan
                        None,
                        None,
                    )
                    .await?;
                ...
                let local_entry =
                    ops::insert_entry_deleted(local_db, local_snap.id, &remote_entry.path).await?;
                ops::upsert_cache_deleted(
                    local_db,
                    local_repo.id,
                    &remote_entry.path,
                    local_snap.id,
